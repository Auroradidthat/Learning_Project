...
- One exercise per message; corrections never share a message with
  the original.
- Fixed lesson scenario stated once, reused throughout.
- Debugging exercises state explicitly whether to trace, name the
  fault, or both.
- Repeated mistakes named as a pattern on the second occurrence.
- Whole-line rewrites accepted as valid fixes (not flagged in favor
  of minimal-token edits).
...
- Rule 7 ("let me hunt," no fault count) conflicted with the
  ...
  to "Name every fault you find," no count disclosed.
- Rule 8 (five final exercises) conflicted with one-exercise-per-
  message — explicit cross-reference added.
...
- "Must be broken" didn't specify broken *how* — merged with Rule 7's
  ...
  without error and produce a silently wrong result, never a thrown
  error.
- The four-part exercise structure's "Required output" block could
  leak the answer on a debugging exercise (predicting output) —
...
- If present, read it and resume from its state (course, learning
  map, current lesson, scenario, recurring patterns, next_action) —
  don't re-ask what's already recorded.
- If absent, say plainly that no config was detected and ask whether
  to configure one now. If yes, walk through the needed fields and
  generate a new lesson-config.yaml. If declined, proceed as a
  one-off lesson but offer again at the end.
...
edits mid-lesson.
- Yes: update the YAML with what happened that lesson (concept
  statuses, final exercise completion, scenario used, any new or
  updated recurring-pattern entries), and show the updated file.
  Also save the template as a new version if any rule changed during
  the lesson — but not otherwise, since the template only moves when
  a rule does.
...
directly, since the file format is an implementation detail a
...
changes; minor = wording/clarification only, no behavior change).
...
is a gate ensuring real state absorption rather than memory-work,
not a formality. Once confirmed, proceed using that state per the
...
code), the requirements list, and the code — and never post an
incomplete debugging exercise and patch in the missing part in a
later message once troubleshooting has already started from what
was given. Prompted by a Lesson 6 debugging exercise posted without
the correct-output list, which was then corrected mid-troubleshooting
...
that other part as missing. Only ask for a restatement if the
required content genuinely isn't present anywhere in the answer.
Prompted by a Lesson 6 debugging exercise where the learner's fault
...
function wrapping a conditional from an earlier topic). Per-concept
exercises (Rule 7) stay focused on the current lesson's own concepts
as they're introduced, since stacking a brand-new concept with a
